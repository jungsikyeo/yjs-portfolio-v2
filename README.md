...
> 여정식의 두 번째 포트폴리오 사이트. 경력 → 프로젝트 → 실제 사용 기술의 연결을 한눈에 탐색하는 APM 콘셉트로 다시 만들었습니다.
...
- **Notion이 곧 콘텐츠**: 경력, 프로젝트, 기술, 자격을 Notion 데이터베이스 5개에서 읽어 D1에 스냅샷으로 저장합니다. Notion만 고치면 방문 시 사이트가 따라옵니다. 빌드나 재배포가 필요 없습니다.
- **나열이 아닌 연결**: 회사와 프로젝트를 목록으로 보여주는 대신 프로필 → 경력 → 프로젝트 → 기술을 토폴로지 그래프로 잇고, 노드를 고르면 관련 관계만 강조합니다.
- **사이트 안에서 끝나는 동선**: 연락 폼과 이력서 PDF 다운로드를 내장했습니다. 이력서는 사이트와 같은 Notion 데이터를 브라우저에서 A4 문서로 만듭니다.
- **서버리스 한 벌**: Cloudflare Workers 하나에 D1(캐시·문의 저장), R2(스크린샷 미러), Email Routing(문의 릴레이)을 바인딩해 별도 서버 없이 운영합니다.
