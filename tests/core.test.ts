...
test('규모 and the labeled 성과 line become their own sections beside 판단 instead of being swallowed by it',()=>{const body=['역할 · 단독 개발','기간 · 2026.06 — 현재','배경','만든 이유','한 일','한 것 1','한 것 2','판단','고른 이유','규모','1,256커밋 중 1,228커밋','성과 · 4종 배포'];const s=narrativeSections({body,role:'단독 개발',period:'2026.06 — 현재'});assert.deepEqual(s.decisions,[{title:'판단',lines:['고른 이유']},{title:'규모',lines:['1,256커밋 중 1,228커밋']},{title:'성과',lines:['4종 배포']}]);assert.deepEqual(s.process,[{title:'한 일',lines:['한 것 1','한 것 2']}]);assert.deepEqual(s.overview.map(b=>b.title),['배경']);});
