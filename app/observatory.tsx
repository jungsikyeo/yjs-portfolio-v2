...
{section==='Overview'&&<section className="intro"><div className="profile-intro"><ProfilePhoto name={data.name} demo={data.demo}/><div className="profile-copy"><span className="tiny-label profile-role">{data.role}</span><h2>{data.name}</h2><p>{data.intro}</p><div className="contact-links">{data.email&&<button type="button" className="contact-button" onClick={()=>setContact(true)}><Mail size={13}/>연락하기</button>}{data.links?.map(link=><a key={link.url} href={link.url} target="_blank" rel="noreferrer">{link.label} <ArrowUpRight size={12}/></a>)}{!sourceData.demo&&<ResumeButton data={sourceData}/>}</div></div></div><div className="metrics">{(['experience','project','skill'] as const).map((kind,i)=><div key={kind}><span className="metric-label">{kind==='experience'&&!data.demo?'주요 경력':kinds[kind]} <span className="metric-code">0{i+1}</span></span><strong>{String(data.entries.filter(e=>e.kind===kind).length).padStart(2,'0')}<small>{kind==='skill'?'개 기술':kind==='experience'&&!data.demo?'곳':'개'}</small></strong><span>{data.demo?'데모 데이터 기준':kind==='skill'?'주요 경력·프로젝트 기술':kind==='experience'?'최근 주요 경력 기준':'표시된 프로젝트 기준'}</span></div>)}</div></section>}
