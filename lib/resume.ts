...
export function buildResume(data:Portfolio,now=new Date()):ResumeDoc{
 ...
 if(experiences.length)sections.push({title:'경력',layout:'block',items:experiences.map(e=>({
  title:e.title,meta:e.period,role:cleanRole(e.role),summary:e.body[0],bullets:e.body.slice(1,1+FEATURED_BULLETS),
 ...
 if(featured.length)sections.push({title:'주요 프로젝트',layout:'block',items:featured.map(p=>({
  title:p.title,meta:p.period,role:cleanRole(p.role),company:parentsOf(p).map(titleOf).filter(t=>experiences.some(e=>e.title===t)).join(' · ')||undefined,summary:p.subtitle,
  bullets:narrativeSections(p).process.flatMap(b=>b.lines).slice(0,FEATURED_BULLETS),stack:skillsOf(p.tags),url:p.url??p.links?.[0]}))});
 if(others.length)sections.push({title:'기타 프로젝트',layout:'compact',items:others.map(p=>({title:p.title,meta:p.period,summary:p.subtitle,bullets:[]}))});
 ...
 const contacts=[...(data.email?[{label:data.email,url:`mailto:${data.email}`}]:[]),...(data.links??[]).map(l=>({label:l.label,url:l.url}))];
