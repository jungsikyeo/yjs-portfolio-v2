...
export type Entry = {id:string; kind:Kind; title:string; subtitle:string; body:string[]; parent?:string; tags:string[]; url?:string; links?:string[]; period?:string; category?:string; role?:string; featured?:boolean};
...
export function edges(entries:Entry[]){return entries.flatMap(e=>[...(e.parent&&entries.some(n=>n.id===e.parent)?[{from:e.parent,to:e.id,label:e.kind==='experience'?'경력':'참여'}]:[]),...e.tags.filter(t=>entries.some(n=>n.id===t)).map(t=>({from:e.id,to:t,label:'사용 기술'}))]);}
